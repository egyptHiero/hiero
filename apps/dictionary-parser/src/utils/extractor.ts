...
export const extract = async (
  ...
  try {
    ...
    const promise = isWithWorkers
      ? runWorkerAsPromise<[string, PDFExtractOptions], PDFExtractResult>(
          workerPath,
          fileName,
          options,
        )
    ...
    return promise
      ...
      .then(() => {
        consoleProgress[fileName].success(
          `finished parsing input file ${fileName}`,
        );
      });
  } catch (e) {
    consoleProgress[fileName].error(`error processing file ${fileName}: ${e}`);
    throw e;
  }
};
