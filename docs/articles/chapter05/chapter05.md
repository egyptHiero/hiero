...
Может это не лучший выбор - она использует блокирующие вызовы и не использует потоки. Но словари не такие большие,
чтобы это стало проблемой.ы
...
все оказалось сравнительно легко - данные в нем были расположены построчно. А вот с табличным видом пришлось
повозиться. В результате я написал утилиту еще одну calculateBoundaries, которая определяет примерные границы столбцов в
таблице. Примерные, потому что текстовый блок в pdf может быть размещен произвольным образом и позиционирование в нём
тоже может быть произвольным. Получив примерные границы колонок, я распарсил, наконец, и оба оставшихся файла.
...
Формат `JSON` хорош, но совсем не годится для потоковой обработки сериализованных объектов. Гораздо лучше подходит формат
[`NDJSON`](https://github.com/ndjson/ndjson.js),
в котором каждая строка это отдельный json. Такой формат удобно читать и записывать в него по частям.
...
Вот так можно запустить парсинг словаря ancient с 2-й по 5-ую страницы в режиме расчета границ колонок -
```bash
