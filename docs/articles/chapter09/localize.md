...
Хорошо, когда приложение имеет возможность для выбора языка. Добавим интернализацию (сокращенно пишется `i18n`).
...
- `ChainedBackend` - позволяет объединить в цепочку несколько бекендов.
...
Загружая `i18next` из конфига мы запускаем инициализацию, но не дожидаемся, когда она закончится.
Поэтому, нельзя использовать функцию перевода из ~~`import { t } from "i18next";`~~, она может быть вызвана до
