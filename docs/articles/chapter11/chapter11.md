...
`Nx` предоставляет гибкие возможности для настройки процессов сборки, разработки и деплоя через конфигурационные файлы.
...
[api, web], буду вызывать его в контексте 'tools'.
...
пересоздаваться при изменении роутов или DTO. Осталось только создать тайпинги с помощью
